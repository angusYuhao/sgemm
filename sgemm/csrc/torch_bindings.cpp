...
// borrowed from: https://docs.pytorch.org/tutorials/advanced/cpp_custom_ops.html
extern "C" {
...
void print_sgemm_version_error() {
    std::cout << "Invalid SgemmVersion!\n";
}
...
void sgemm_cuda(const torch::Tensor& A, const torch::Tensor& B,
                torch::Tensor& C, double alpha, double beta, int64_t version) {
    ...
    switch(sgemm_version) {
        case SgemmVersion::sgemm_naive:
            sgemm_naive_cuda(M, N, K, (float*)(A.data_ptr()),
                             (float*)(B.data_ptr()), (float*)(C.data_ptr()),
                             static_cast<float>(alpha),
                             static_cast<float>(beta));
            break;
        case SgemmVersion::sgemm_global_memory_coalesce_v1:
            sgemm_global_memory_coalesce_v1_cuda(M, N, K,
                                                 (float*)(A.data_ptr()),
                                                 (float*)(B.data_ptr()),
                                                 (float*)(C.data_ptr()),
                                                 static_cast<float>(alpha),
        ...
        case SgemmVersion::sgemm_global_memory_coalesce_v2:
            sgemm_global_memory_coalesce_v2_cuda(M, N, K,
                                                 (float*)(A.data_ptr()),
                                                 (float*)(B.data_ptr()),
                                                 (float*)(C.data_ptr()),
                                                 static_cast<float>(alpha),
...
TORCH_LIBRARY(sgemm, m) {
    m.def(
        "sgemm(Tensor A, Tensor B, Tensor C,"
        "      float alpha, float beta, int version) -> ()"
    );
}

TORCH_LIBRARY_IMPL(sgemm, CUDA, m) {
    m.impl("sgemm", &sgemm_cuda);
}
